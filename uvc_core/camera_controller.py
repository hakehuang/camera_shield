...
class UVCCamera:
    ...
    def show_analysis(self, analysis, frame=None):
        ...
        print(
            f"  Color Balance - A Channel: {analysis['color_balance']['a_channel']:.2f}"
        )
        print(
            f"  Color Balance - B Channel: {analysis['color_balance']['b_channel']:.2f}"
        )
    ...
    def analyze_multiple_frames(self, num_frames=10):
        ...
        avg_analysis = {
            ...
            "color_balance": {
                "a_channel": np.mean(
                    [r["color_balance"]["a_channel"] for r in results]
                ),
                "b_channel": np.mean(
                    [r["color_balance"]["b_channel"] for r in results]
                ),
            },
    ...
    def print_settings(self):
        ...
        print(
            f"  Auto Focus: {'ON' if self.cap.get(cv2.CAP_PROP_AUTOFOCUS) else 'OFF'}"
        )
        print(
            f"  Auto White Balance: {'ON' if self.cap.get(cv2.CAP_PROP_AUTO_WB) else 'OFF'}"
        )
