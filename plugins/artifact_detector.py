...
class ArtifactDetector(DetectionPlugin):
    def __init__(self, config):
        super().__init__('ArtifactDetector')
    ...
    def process_frame(self, frame):
        ...
        try:
            ...
            colored = cv2.applyColorMap(cv2.convertScaleAbs(norm_mag, alpha=1.5), cv2.COLORMAP_JET)
            
            # 叠加显示
            overlay = cv2.resize(colored, (display_size, display_size))
            cv2.addWeighted(
                overlay, 0.7,
                frame[10:10+display_size, 10:10+display_size], 0.3,
                0,
            ...
            # 绘制绿色边框
            cv2.rectangle(frame, 
                (10,10), (10+display_size,10+display_size),
                (0,255,0), 2
            )
        ...
        if self.frame_counter % self.detection_interval == 0:
            ...
            if alerts:
                ...
                cv2.putText(frame, f'Alerts: {', '.join(alerts)}', 
                    (display_size+30, 30), cv2.FONT_HERSHEY_SIMPLEX,
                    0.7, (0, 0, 255), 2
                )
