...
class PatternNoiseDetector(DetectionPlugin):
    def __init__(self, config):
        super().__init__('PatternNoiseDetector')
        self.thresholds = {
    ...
    def _calculate_pattern_score(self, frame):
        ...
        # 4. 计算模式得分
        pattern_score = high_freq / (low_freq + 1e-5)
        return pattern_score

    def process_frame(self, frame):
        """处理视频帧，检测花屏"""
        # 计算当前帧的模式得分
        current_score = self._calculate_pattern_score(frame)
        ...
        # 判断是否为花屏
        is_pattern_noise = current_score > self.thresholds['pattern_score']
        
        # 在画面上显示检测结果

        # 在右下角显示检测结果和得分
        cv2.putText(frame, f"Pattern Noise: {'Detected' if is_pattern_noise else 'Normal'}", 
                    (frame.shape[1]-300, frame.shape[0]-60), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        cv2.putText(frame, f"Score: {current_score:.2f}", 
